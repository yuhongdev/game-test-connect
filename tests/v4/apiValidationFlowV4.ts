/**
 * apiValidationFlowV4.ts — Fully concurrent game validation (v4, memory-safe).
 *
 ...
 *
 *  All vendor CSVs from a single test run are stored in a shared datetime folder:
 *
 *    test-results/vendor-reports/
 *        2026-03-19T08-24-15/           ← one folder per run
 ...
 *            ...
 *
 *  The run folder name is the RUN_TIMESTAMP passed in from the spec file.
 *  It is computed ONCE at spec load time (before any test starts), so all
 *  53 vendor tests from the same run share the same folder.
 *
 *  This replaces the previous flat structure where all vendor CSVs lived in
 *  one folder mixed together, making it impossible to tell which files came
 *  from the same run.
 *
 * ── Fix history ───────────────────────────────────────────────────────────────
 *
 ...
 *  FIX 11 — per-run datetime folder (all vendors in same run share one folder)
 *
 * ── Run commands ──────────────────────────────────────────────────────────────
 *
 *  All vendors:    npx playwright test tests/v4/ --project=chromium --workers=6
 *  Single vendor:  npx playwright test tests/v4/ --project=chromium -g "v4: Amusnet" --workers=1 --headed
...
/**
 * Base directory for all CSV output.
 * CSVs go into a per-run subfolder: REPORTS_DIR/<RUN_TIMESTAMP>/
 *
 * This lives inside test-results/ because that is where all test artefacts
 * belong. Note: Playwright wipes test-results/ before each run, which is
 * actually desirable here — old run folders are cleaned up automatically.
 * If you want to KEEP historical runs across multiple test sessions, move this
 * to a folder outside test-results/, e.g.:
 *   path.resolve(__dirname, '..', '..', 'vendor-reports')
 */
 ...
 * @param runTimestamp Shared run folder name, e.g. "2026-03-19T08-24-15".
 *                     Generated ONCE per test run by the spec file and passed
 *                     to all vendor calls so they share the same output folder.
 *                     If omitted, a per-vendor timestamp is used (flat structure,
 *                     no run grouping).
 */
export async function apiValidateVendorGamesFlowV4(
    browser:       Browser,
    vendorId:      number,
    vendorName:    string,
    runTimestamp?: string,
    ...
    // FIX 11: use the shared run timestamp if provided, otherwise fall back to
    // a per-vendor timestamp (maintains backward compatibility).
    const timestamp = runTimestamp ?? new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);

    // Build the CSV path: REPORTS_BASE_DIR/<timestamp>/<VendorName>_<timestamp>.csv
    ...
    console.log(`\n=== [${vendorName}] v4 validation starting ` +
