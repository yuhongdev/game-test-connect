/**
 * s9_test_v3.spec.ts — v3 test runner: fully concurrent game validation.
 *
 * Architecture vs v1 (s9_test.spec.ts):
 *
 *  v1: 1 browser page per vendor — games loop sequentially within the page.
 *
 *  v3: games run with a semaphore-based concurrent queue (no fixed batches).
 ...
 *
 * ── Memory usage ──────────────────────────────────────────────────────────────
 *  Each browser page uses ~300MB.
 *  At 14 workers × 6 games = 84 pages → ~16.8GB peak.
 *  Safe for machines with 33GB RAM.
 *
 * ── Run commands ──────────────────────────────────────────────────────────────
 *  All vendors:
 ...
 *    npx playwright test tests/v3/ --project=chromium -g "v3: Amusnet" --workers=1 --headed
 *
...
import { apiValidateVendorGamesFlowV3 } from './apiValidationFlowV3';
...
function loadVendors(): Array<{ id: number; name: string }> {
    if (!fs.existsSync(VENDORS_FILE)) {
        throw new Error(
            ...
            `Then retry: npx playwright test tests/v3/ --project=chromium`
        );
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// v3 API Vendor Validation Tests
//
...
//
// Run all vendors:     npx playwright test tests/v3/ --project=chromium --workers=14
// Run single vendor:   npx playwright test tests/v3/ --project=chromium -g "v3: Amusnet"
// ─────────────────────────────────────────────────────────────────────────────
test.describe('v3 API Vendor Validation', () => {
    ...
    for (const vendor of vendors) {
        // Test name prefixed with "v3:" to distinguish from v1 tests in the report
        test(`v3: ${vendor.name} (ID: ${vendor.id})`, async ({ browser }) => {
            await apiValidateVendorGamesFlowV3(browser, vendor.id, vendor.name);
        });
    }
});
